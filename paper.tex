...
\subsection{Preliminary Empirical Validation of Tractability}
\label{sec:tractability-validation}

To verify the mathematical soundness and engineering feasibility of the framework, we implemented the AIF-DPO meta-optimization loop using the HuggingFace Transformers and PEFT \citep{peft} libraries. Using a 135-million-parameter causal language model (\texttt{SmolLM2-135M-Instruct}) augmented with Low-Rank Adaptation (LoRA), we validated the following structural claims:

\begin{enumerate}
    \item \textbf{Native EFE Computation:} Information Gain ($\mathcal{I}$, analogized directly from the predictive distribution) and Expected Free Energy map seamlessly onto standard next-token probability tensors, confirming the claims of \cref{sec:vfe}.
    \item \textbf{Preference Dataset Construction (\cref{sec:efe-pairs}):} The model successfully evaluated generated candidates based on simulated observations and dynamically constructed EFE-ranked preference pairs in accordance with \cref{eq:pair-selection}, isolating ''informative'' vs.~''redundant'' trajectories precisely without external supervision.
    \item \textbf{Stable Gradient Flow:} The EFE-derived pairs fed directly into the AIF-DPO loss objective (\cref{eq:aif-dpo}). Gradients propagated gracefully backward through the objective, LoRA adapters, and transformer heads, establishing that this active-inference update remains stable without exploding gradients.
\end{enumerate}

These preliminary results provide engineering proof that the framework is computationally viable natively within standard foundation model infrastructure, avoiding the need for heavy, bespoke reinforcement learning scaffolds.
